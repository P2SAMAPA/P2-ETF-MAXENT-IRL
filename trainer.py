"""
trainer.py  —  Orchestrator for Maximum Entropy IRL Engine
===========================================================

Loads data → learns reward functions → ranks ETFs → builds JSON.
...
from maxent_irl import compute_universe_maxent_irl
...
def process_window(args: Tuple) -> Dict:
    ...
    try:
        ...
        result = compute_universe_maxent_irl(universe_prices, macro_df, config_dict, window)
...
def run_trainer(hf_token: Optional[str] = None) -> Dict:
        ...
        for ticker in available:
            ...
            if best_win is not None:
                best_window_per_etf[ticker] = {
                    "z_score": best_z,
                    "window": int(best_win),
                    "reward": safe_float(best_data.get("reward", 0)),
                    "n_iterations": int(safe_float(best_data.get("n_iterations", 0))),
                    "converged": best_data.get("converged", False),
    ...
    logger.info("\n💾 Saving JSON results...")
    tab1_path = f"maxent_irl_{run_date}.json"
    tab2_path = f"maxent_irl_breakdown_{run_date}.json"
