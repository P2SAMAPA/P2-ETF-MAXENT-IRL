...
st.set_page_config(page_title="MaxEnt-IRL Engine", layout="wide")
...
.badge-strongsell{background:#7a1a1a;border-radius:6px;padding:2px 12px;font-size:0.75rem;
                  font-weight:700;color:white}
.badge-irl{background:#8e44ad;border-radius:6px;padding:2px 8px;font-size:0.65rem;
           font-weight:700;color:white}
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="main-header">🧠 MaxEnt-IRL Engine</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">Maximum Entropy Inverse Reinforcement Learning · '
    'Reward Function Inference · Inferring Trading Goals</div>',
    unsafe_allow_html=True)
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
st.sidebar.markdown("## 🧠 MaxEnt-IRL")
st.sidebar.markdown(f"**Next Trading Day**")
st.sidebar.markdown(f"`{next_trading_day()}`")
st.sidebar.markdown(f"**IRL Iterations:** {config.IRL['n_iterations']}")
st.sidebar.markdown(f"**Learning Rate:** {config.IRL['learning_rate']}")
st.sidebar.markdown(f"**Trajectories:** {config.IRL['n_trajectories']}")
st.sidebar.markdown("---")
st.sidebar.markdown("**Features:**")
for feature, enabled in config.FEATURES.items():
    st.sidebar.markdown(f"  • {feature}: {'✅' if enabled else '❌'}")
st.sidebar.markdown("---")
...
tab1_path = find_latest(files, "maxent_irl_")
tab2_path = find_latest(files, "maxent_irl_breakdown_")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.header("🏆 Best Window per ETF — MaxEnt-IRL Signal")

    with st.expander("📖 How MaxEnt-IRL Works", expanded=True):
        st.markdown("""
**Maximum Entropy Inverse Reinforcement Learning** infers trading goals from market data:

| Step | What happens |
|------|-------------|
| 1. Extract features | Extract momentum, volatility, drawdown, etc. from market data |
| 2. Sample trajectories | Create trajectories of market behavior |
| 3. Learn reward | Infer the reward function that best explains observed behavior |
| 4. Rank ETFs | Higher reward = better alignment with market goals |

**Interpretation:**
- **Higher z-score** → ETF aligns well with inferred market goals
- **Reward** → Inferred value of the asset
- **Converged** → IRL algorithm converged successfully
        """)
    ...
    for universe_name in UNIVERSE_ORDER:
        ...
        if buy_etfs:
            ...
            for idx, (ticker, z_score, data) in enumerate(buy_etfs[:3]):
                ...
                reward = safe_float(data.get("reward", 0))
...
<div class="hero-card">
  ...
  <div class="score">Reward = {reward:.3f}</div>
...
""", unsafe_allow_html=True)
            ...
            if full_scores:
                ...
                for t, info in full_scores.items():
                    ...
                    rows.append({
                        "ETF": t,
                        "z-score": round(z, 4),
                        "Reward": round(safe_float(info.get("reward", 0)), 4),
                        "N Iterations": int(safe_float(info.get("n_iterations", 0))),
                        "Converged": "✅" if info.get("converged", False) else "❌",
    ...
    st.caption(f"Run date: {data1.get('run_date','?')} · Higher z-score = better alignment with inferred market goals")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.header("🔍 Explore MaxEnt-IRL by Window")
...
- **252d** — Annual reward inference: recommended primary
- **504d** — Long-term reward inference
        """)

    st.markdown(f"### MaxEnt-IRL Rankings at **{win_labels.get(selected_win, f'{selected_win}d')}** window")
